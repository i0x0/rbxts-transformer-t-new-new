...
const instanceDefType = path.normalize(
	path.join(typePath, "include", "generated", "None.d.ts"),
);
...
function get_t_Path(): string {
	try {
		...
		throw "[rbxts-transformer-t-new-new ERROR]: @rbxts/t must be installed for rbxts-transformer-t-new-new to work.";
	}
}
...
function createPropertyAccess(
	...
	return factory.createPropertyAccessExpression(
		factory.createIdentifier(OBJECT_NAME),
...
function createMethodCall(
	...
	return factory.createCallExpression(
		createPropertyAccess(methodName),
		undefined,
		params,
	);
}
...
function convertUnionType(
	...
	const [literalTypes, notLiteralTypes] = utility.separateArray(
		types,
		utility.isLiteral(typeChecker),
	);
	...
	const literalExpression =
		literalTypes.length === 0
			...
			: factory.createCallExpression(
					createPropertyAccess("literal"),
					undefined,
					literalTypes.map(
						(type) =>
							createLiteralExpression(type, typeChecker) as ts.Expression,
					),
				);

	const result = convertTypesArray(notLiteralTypes, typeChecker);

	if (result.length === 0 && literalExpression) return literalExpression;

	if (literalExpression) result.push(literalExpression);
...
function convertTupleType(
	...
	const result = convertTypesArray(
		(<any>type).resolvedTypeArguments,
		typeChecker,
	);
...
function convertMapType(
	...
	if (args === undefined) throw new Error("Map must have type arguments");
...
function convertObjectType(
	...
	if (props.length === 0)
		return createMethodCall("interface", [
	...
	const [optionalProps, nonOptionalProps] = utility.separateArray(
		preparedProps,
		utility.isOptionalPropertyDeclaration,
	);

	// Builds t.interface entity by property list
	const handlePropList = (
		props: ts.Symbol[],
		isOptional?: boolean,
	): ts.Expression => {
		const handledProps = props.map((prop) =>
			utility.extractProperty(prop, typeChecker),
		);
		...
		const properties = result.map((p, i) =>
			factory.createPropertyAssignment(
				utility.buildPropertyName(handledProps[i].name),
				isOptional ? createMethodCall("optional", [p]) : p,
			),
		);
...
function convertInterfaceType(
	...
	if (parents.length === 0) return object;
...
let tTypeDefinitions = fs.readFileSync(
	path.join(get_t_Path(), "lib", "t.d.ts"),
	"utf8",
);
...
export function is_t_ImportDeclaration(program: ts.Program) {
	return (node: ts.Node) => {
		...
		if (
			!importSymbol ||
			importSymbol.valueDeclaration!.getSourceFile().text !== tTypeDefinitions
		)
			// TODO
			return false;
...
export function buildType(
	...
	if (fileName && path.normalize(fileName) === instanceDefType) {
		return createMethodCall("instanceIsA", [
			factory.createStringLiteral(stringType),
		]);
	}

	if (utility.isBrickColorType(type)) return createPropertyAccess("BrickColor");

	if (utility.isEnum(type))
		return createMethodCall("enum", [
	...
	if (stringType === "true" || stringType === "false") {
		const literal =
			stringType === "true" ? factory.createTrue() : factory.createFalse();
	...
	if (utility.isFunctionType(type)) return createPropertyAccess("callback");

	// Complex types transformation
	try {
		if (utility.isCustomEnum(type)) return convertEnumType(type, typeChecker);

		if (utility.isMapType(type)) return convertMapType(type, typeChecker);

		if (type.isUnion()) return convertUnionType(type, typeChecker);
